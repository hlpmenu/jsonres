...
func ServeUnauthorized(w http.ResponseWriter) {
	ServeJsonStatusRes("error", "unauthorized", http.StatusUnauthorized, w)
}
...
func ServeInternalServerError(w http.ResponseWriter) {
	ServeJsonStatusRes("error", "internal server error", http.StatusInternalServerError, w)
}
...
func ServeNotFound(w http.ResponseWriter) {
	ServeJsonStatusRes("error", "not found", http.StatusNotFound, w)
}
...
func ServeBadRequest(w http.ResponseWriter) {
	ServeJsonStatusRes("error", "bad request", http.StatusBadRequest, w)
}
...
func ServeForbidden(w http.ResponseWriter) {
	ServeJsonStatusRes("error", "forbidden", http.StatusForbidden, w)
}
...
func Example(w http.ResponseWriter, status, message string) {
	ServeJsonStatusRes(status, message, http.StatusUnauthorized, w)
}
